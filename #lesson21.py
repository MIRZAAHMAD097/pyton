...
# Exercise 4: Bar chart for subject averages
df1[['Math', 'English', 'Science']].mean().plot(kind='bar', title='Average per Subject')
plt.ylabel('Score')
...
top_sales_date = df2.loc[df2['Total'].idxmax(), 'Date']

# Exercise 3: Percentage change in sales per product
df2[['A_%Change', 'B_%Change', 'C_%Change']] = df2[['Product_A', 'Product_B', 'Product_C']].pct_change() * 100

# Exercise 4: Sales trend line chart
...
# Exercise 4: Employee count by department
...
df4.groupby('Product')['Total_Price'].sum().plot(kind='pie', autopct='%1.1f%%', title='Sales by Product')
plt.ylabel('')
plt.tight_layout()
plt.show()
